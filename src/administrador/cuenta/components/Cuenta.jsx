  ...
  const eliminar = async (id) => {
    try {
      ...
      if (confirmacion.isConfirmed) {
        ...
        const cuentasActualizadas = cuentas.filter((cuenta) => cuenta._id !== id);
                  ...
                  <td>
                    <div className="d-grid gap-2 col mx-auto">
                      <div className="d-grid gap-2">
                        <Button onClick={() => eliminar(cuenta._id)} className="btn btn-danger" style={{ backgroundColor: "#CD5C5C", border: "none" }}>
                        <i className="fa fa-trash mx-2"></i>Eliminar
                        </Button>
                        <Link to={`/historialAdmin/${cuenta._id}`} className="nav-link">
                          <div className="d-grid gap-2 col-190 mx-auto" style={{ marginBottom: "10px" }}>
                            <div className="btn btn-primary btn-ver-historial" type="button">
                            <i className="fa fa-user mx-2"></i>Ver Historial
                            </div>
                          </div>
                        </Link>
                      </div>
